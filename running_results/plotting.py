...
class RacePlotter:
    ...
    def plot_gender_comparison(
        ...
        if gender_column == 'Category':
            male_data = data[data[gender_column].apply(lambda x: str(x)[-1] == male_value if pd.notna(x) else False)]
            female_data = data[data[gender_column].apply(lambda x: str(x)[-1] == female_value if pd.notna(x) else False)]
        else:
        ...
        if self.use_kentigern_style:
            with KentigernPlot(1, 1, figsize=figsize or (4, 1)) as fig:
                ...
                ax.fill_between(bins, 100 * kde_male(bins), alpha=0.3, color='blue', label='Male')
                ax.fill_between(bins, 100 * kde_female(bins), alpha=0.3, color='red', label='Female')
    ...
    def plot_cumulative_distribution(
        ...
        if self.use_kentigern_style:
            with KentigernPlot(1, 1, figsize=figsize or (4, 2)) as fig:
                ...
                if len(datasets) > 1:
                    ax.legend(loc="center right", prop={"size": 6, "family": "Consolas"})
    ...
    def plot_histogram(
        ...
        if self.use_kentigern_style:
            with KentigernPlot(1, 1, figsize=figsize or (4, 2)) as fig:
                ...
                ax.hist(data[time_column].dropna(), bins=bins, alpha=0.5, color='#79C4F2')
                ax.set(yticklabels=[])
                ax.tick_params(left=False)
                ax.set_xlabel("Finish Time (minutes)")
    ...
    def plot_club_comparison(
        ...
        if self.use_kentigern_style:
            with KentigernPlot(1, 2, figsize=figsize or (8, 2)) as fig:
                ...
                ax.fill_between(bins, 100 * kde_club(bins), alpha=0.5, color='red', label='In club')
                ax.fill_between(bins, 100 * kde_non_club(bins), alpha=0.5, color='blue', label='No club')
                ax.plot(bins, 100 * kde_club(bins), alpha=0.5, color="black", linewidth=1,
                       path_effects=[pe.Stroke(linewidth=1.5, foreground='w'), pe.Normal()])
                ax.plot(bins, 100 * kde_non_club(bins), alpha=0.5, color="black", linewidth=1,
                       path_effects=[pe.Stroke(linewidth=1.5, foreground='w'), pe.Normal()])
                ...
                # Cumulative plot
                ax = fig.axes[1]
                ax.hist(club_runners[time_column].dropna(), bins=bins, cumulative=True, density=True,
                       histtype='step', color="red", linewidth=1,
                       path_effects=[pe.Stroke(linewidth=1.5, foreground='w'), pe.Normal()])
                ax.hist(non_club_runners[time_column].dropna(), bins=bins, cumulative=True, density=True,
                       histtype='step', color="blue", linewidth=1,
                       path_effects=[pe.Stroke(linewidth=1.5, foreground='w'), pe.Normal()])
            ...
            # Cumulative plot
            ax = axes[1]
            ax.hist(club_runners[time_column].dropna(), bins=bins, cumulative=True,
                   density=True, histtype='step', label='In club')
            ax.hist(non_club_runners[time_column].dropna(), bins=bins, cumulative=True,
                   density=True, histtype='step', label='No club')
            ax.yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1))
            ax.set_xlabel("Finish Time (minutes)")
            ax.set_ylabel("Percentage finished")
            ax.legend()
