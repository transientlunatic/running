...
def create_app(
    config: Optional[APIConfig] = None, db_path: Optional[str] = None
) -> Flask:
    """
...
def register_routes(app: Flask) -> None:
    ...
    def get_runner_history(name: str):
        ...
        try:
            ...
            results_df = db.get_runner_history(runner_name=name, race_name=race_name)

            if results_df.empty:
                return (
                    jsonify(
                        {"error": "No results found for runner", "runner_name": name}
                    ),
                    404,
                )
...
def register_error_handlers(app: Flask) -> None:
    ...
    def bad_request(error):
        return (
            jsonify({"error": "Bad request", "message": "The request was invalid"}),
            400,
        )
