...
def require_api_key(f: Callable) -> Callable:
    ...
    def decorated_function(*args, **kwargs):
        ...
        # 1. Check X-API-Key header (recommended)
        ...
        # 3. Check query parameter (DEPRECATED - security risk)
        # Query parameters may be logged in server logs, browser history, etc.
        if not api_key:
            ...
            if api_key:
                ...
                warnings.warn(
                    "Using API key in query parameters is deprecated and insecure. "
                    "Use X-API-Key header or Authorization Bearer token instead.",
                    DeprecationWarning,
                    stacklevel=2,
                )
