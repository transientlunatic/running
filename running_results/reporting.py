...
def generate_race_report(
    ...
    if not OTTER_AVAILABLE:
        raise ImportError(
            "otter-report is not installed. Install with: pip install otter-report"
        )
    ...
    if len(finishers) > 0:
        ...
        time_metrics = Grid(
            [
                ...
                Metric(
                    "Slowest Time", f"{finishers['finish_time_minutes'].max():.2f} min"
                ),
            ]
        )
    ...
    if len(finishers) > 0:
        ...
        top_10["finish_time_minutes"] = top_10["finish_time_minutes"].apply(
            lambda x: f"{x:.2f}"
        )
    ...
    if (
        "age_category" in finishers.columns
        and finishers["age_category"].notna().sum() > 0
    ):
        category_section = Section("Age Category Analysis")

        category_stats = (
            finishers.groupby("age_category")
            .agg({"finish_time_minutes": ["count", "mean", "min"]})
            .round(2)
        )
...
def generate_comparison_report(
    results: pd.DataFrame, race_name: str, output_path: str = "comparison_report.html"
) -> None:
    """
...
def generate_runner_report(
    history: pd.DataFrame, runner_name: str, output_path: str = "runner_report.html"
) -> None:
    """
    ...
    if finishes > 0:
        ...
        perf_metrics = Grid(
            [
                Metric(
                    "Best Time", f"{finishers['finish_time_minutes'].min():.2f} min"
                ),
                Metric(
                    "Average Time", f"{finishers['finish_time_minutes'].mean():.2f} min"
                ),
    ...
    history_display["finish_time_minutes"] = history_display[
        "finish_time_minutes"
    ].apply(lambda x: f"{x:.2f}" if pd.notna(x) else "DNF")
