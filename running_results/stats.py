...
class RaceStatistics:
    ...
    def summary_statistics(self, column: str = "FinishTime (minutes)") -> pd.Series:
        """
    ...
    def percentile_for_time(
        self, time: float, time_column: str = "FinishTime (minutes)"
    ) -> float:
        """
    ...
    def time_for_percentile(
        self, percentile: float, time_column: str = "FinishTime (minutes)"
    ) -> float:
        """
    ...
    def club_comparison(
        self, club_column: str = "Club", time_column: str = "FinishTime (minutes)"
    ) -> pd.DataFrame:
        """
    ...
    def percentile_table(
        ...
        for p in percentiles:
            ...
            results.append({"Percentile": f"{p}%", "Time (minutes)": round(time, 2)})
    ...
    def year_over_year_comparison(
        ...
        comparison = pd.DataFrame({"count": grouped.count(), stat: grouped.agg(stat)})
...
class RaceComparison:
    ...
    def compare_summary(self, column: str = "FinishTime (minutes)") -> pd.DataFrame:
        """
