...
class TestAddResultsEndpoint:
    ...
    def test_add_results_without_api_key(self, client):
        ...
        response = client.post(
            "/api/results", json={"race_name": "Test Race", "results": []}
        )
    ...
    def test_add_results_with_api_key_header(self, client):
        ...
        response = client.post(
            "/api/results", json=new_results, headers={"X-API-Key": "test-api-key"}
        )
    ...
    def test_add_results_with_api_key_query_param(self, client):
        ...
        new_results = {
            "race_name": "New Race",
            "results": [{"name": "Test Runner", "position_overall": 1}],
        }

        response = client.post("/api/results?api_key=test-api-key", json=new_results)
        assert response.status_code == 201
    ...
    def test_add_results_missing_race_name(self, client):
        ...
        response = client.post(
            "/api/results", json={"results": []}, headers={"X-API-Key": "test-api-key"}
        )
    ...
    def test_add_results_with_column_mapping(self, client):
        ...
        new_results = {
            ...
            "results": [{"Pos": 1, "Runner": "Test Runner", "Time": 1800}],
        }
    ...
    def test_add_results_invalid_data(self, client):
        ...
        new_results = {"race_name": "Bad Race", "results": [{"invalid_field": "value"}]}
